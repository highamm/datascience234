...
The code below scales all numeric variables in a data set, using the `across()` function. `across()` applies a tranformation to every column in a data set that satisfies the condition given in the `where` argument.
